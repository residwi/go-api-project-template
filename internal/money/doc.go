...
//
// Add and Sub refuse to combine different currencies, returning
// ErrCurrencyMismatch rather than a silently meaningless number. Equal compares
// amount AND currency. MulQty is the only multiply, because line totals are the
// only place the domain scales money and they always scale by a count.
//
// Three deliberate absences, each of which will look like an oversight: there is
// no Div (dividing money needs a stated rounding and remainder policy), no float
