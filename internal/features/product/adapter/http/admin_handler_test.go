...
func TestAdminHandler_Get(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		prodID := uuid.New()
		now := time.Now()
		service.EXPECT().GetByID(mock.Anything, prodID).Return(&domain.Product{
			ID:        prodID,
			Name:      "Widget",
			Slug:      "widget",
...
func TestAdminHandler_List(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		now := time.Now()
		service.EXPECT().ListAdmin(mock.Anything, mock.Anything).Return([]domain.Product{
			{
				ID:        uuid.New(),
				Name:      "Widget",
				Slug:      "widget",
	...
	t.Run("service error", func(t *testing.T) {
		...
		service.EXPECT().ListAdmin(mock.Anything, mock.Anything).Return(nil, 0, errors.New("db error"))
	...
	t.Run("with valid category_id", func(t *testing.T) {
		...
		catID := uuid.New()
		service.EXPECT().ListAdmin(mock.Anything, mock.MatchedBy(func(p product.AdminListParams) bool {
...
func TestAdminHandler_Create(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		sku := "SKU-999"
		service.EXPECT().
			Create(mock.Anything, mock.Anything, "New Product", mock.Anything, mock.Anything, mock.Anything,
...
func TestAdminHandler_Update(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run("rejects "+tc.name+" with 400", func(t *testing.T) {
			...
			// No service expectation, so mockery fails the test if the request reaches the
	...
	t.Run("accepts price and currency together", func(t *testing.T) {
		...
		prodID := uuid.New()
		service.EXPECT().
			Update(mock.Anything, prodID, mock.Anything, mock.Anything, mock.Anything,
				mock.MatchedBy(func(p *money.Money) bool { return p != nil && *p == money.New(2000, "EUR") }),
				mock.Anything, mock.Anything, mock.Anything).
			Return(&domain.Product{
				ID:     prodID,
				Name:   "Widget",
...
func TestAdminHandler_Delete(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		prodID := uuid.New()
		service.EXPECT().Delete(mock.Anything, prodID).Return(nil)
...
// toAdminProductResponse duplicates toProductResponse's eleven shared field
// mappings by hand, so the fixture sets fields unrelated to SKU and Status
// too, to catch that duplication drifting.
...
func setupAdminMux(t *testing.T) (*http.ServeMux, *MockProductManager) {
	t.Helper()
