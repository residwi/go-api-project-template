...
func TestCache_RoundTrip(t *testing.T) {
	t.Run("returns what Put stored", func(t *testing.T) {
		...
		require.NoError(t, c.Put(ctx, id, user.StatusSnapshot{Active: true, TokenVersion: 42}, time.Minute))

		got, found, err := c.Get(ctx, id)

		require.NoError(t, err)
		assert.True(t, found)
		assert.Equal(t, user.StatusSnapshot{Active: true, TokenVersion: 42}, got)
	})
	...
	t.Run("round-trips an inactive user", func(t *testing.T) {
		...
		require.NoError(t, c.Put(ctx, id, user.StatusSnapshot{Active: false, TokenVersion: 3}, time.Minute))

		got, found, err := c.Get(ctx, id)

		require.NoError(t, err)
		assert.True(t, found)
		assert.Equal(t, user.StatusSnapshot{Active: false, TokenVersion: 3}, got)
	})
}
...
func TestCache_Put(t *testing.T) {
	t.Run("sets the ttl in the same command that writes the fields", func(t *testing.T) {
		...
		require.NoError(t, c.Put(ctx, id, user.StatusSnapshot{Active: true, TokenVersion: 1}, 30*time.Second))
...
func TestCache_Invalidate(t *testing.T) {
	t.Run("removes the entry so a later Get misses", func(t *testing.T) {
		...
		c := New(testRedis)
		id := uuid.New()
		require.NoError(t, c.Put(ctx, id, user.StatusSnapshot{Active: true, TokenVersion: 1}, time.Minute))
