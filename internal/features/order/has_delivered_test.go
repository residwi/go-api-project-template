...
func TestPostgresRepository_HasDeliveredOrder(t *testing.T) {
	...
	seedOrderItem := func(t *testing.T, userID, productID uuid.UUID, status order.Status) {
		t.Helper()
		orderID := uuid.New()
	...
	t.Run("true when the user has a delivered order containing the product", func(t *testing.T) {
		setup(t)
		userID := seedUser(t)
		productID := seedProduct(t)
		seedOrderItem(t, userID, productID, order.StatusDelivered)

		ok, err := repo.HasDeliveredOrder(ctx, userID, productID)
	...
	t.Run("false when the matching order is not delivered", func(t *testing.T) {
		...
		userID := seedUser(t)
		productID := seedProduct(t)
		seedOrderItem(t, userID, productID, order.StatusPaid)
	...
	t.Run("false when the user has no order for the product", func(t *testing.T) {
		setup(t)
		userID := seedUser(t)
		productID := seedProduct(t)
	...
	t.Run("false when the delivered order belongs to another user", func(t *testing.T) {
		...
		other := seedUser(t)
		productID := seedProduct(t)
		seedOrderItem(t, buyer, productID, order.StatusDelivered)

		ok, err := repo.HasDeliveredOrder(ctx, other, productID)
