// Package app_test exercises app.New from outside, the way every
...
func TestMain(m *testing.M) {
	pool, cleanup := testutil.MustStartPostgres("test_app")
	defer cleanup()
	testPool = pool
...
// exception.
//
// This package owns Postgres database "test_app", so it is on the
...
func TestNewWiresOrderAndPaymentToEachOther(t *testing.T) {
	...
	require.NoError(t, deps.Checkout.CancelOrder(ctx, userID, cancelOrderID))
	...
	_, err = deps.Checkout.RetryPayment(ctx, userID, retryOrderID, "card")
