...
func New(repo Repository, cache StatusCache, logger *slog.Logger) *UseCase {
	return &UseCase{repo: repo, cache: cache, logger: logger}
}

func (r *UseCase) GetByID(ctx context.Context, id uuid.UUID) (*domain.User, error) {
	return r.repo.GetByID(ctx, id)
}

func (r *UseCase) ListAdmin(ctx context.Context, params Params) ([]domain.User, int, error) {
	return r.repo.ListAdmin(ctx, params)
}

func (r *UseCase) CheckStatus(ctx context.Context, userID uuid.UUID) (contract.AccountStatus, error) {
