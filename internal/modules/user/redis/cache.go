...
func (c *Cache) Get(ctx context.Context, userID uuid.UUID) (user.StatusSnapshot, bool, error) {
	fields, err := c.rdb.HGetAll(ctx, statusKey(userID)).Result()
	if err != nil {
		return user.StatusSnapshot{}, false, err
	}
	if len(fields) == 0 {
...
var _ user.StatusCache = (*Cache)(nil)
