...
func TestPostgresRepository_GetByID_WithNullableFields(t *testing.T) {
	t.Run("returns payment with payment_method_id", func(t *testing.T) {
		...
		_, err := testPool.Exec(ctx,
			`UPDATE payments SET payment_method_id = $1 WHERE id = $2`, "pm_refund_get_test", p.ID)
		require.NoError(t, err)

		got, err := repo.GetByID(ctx, p.ID)
		require.NoError(t, err)
		assert.Equal(t, "pm_refund_get_test", got.PaymentMethodID)
	})
}
...
func TestPostgresRepository_UpdateStatus(t *testing.T) {
	t.Run("transitions status", func(t *testing.T) {
		...
		_, err := testPool.Exec(ctx, `UPDATE payments SET status = 'success' WHERE id = $1`, p.ID)
		require.NoError(t, err)

		err = repo.UpdateStatus(ctx, p.ID, domain.StatusRefunded, []domain.Status{domain.StatusSuccess})
		require.NoError(t, err)
...
// seedPayment inserts a payment row directly: refund owns no Create method of
// its own (only charge does), so its tests seed through SQL.
