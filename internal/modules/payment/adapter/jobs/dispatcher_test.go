...
func TestDispatcher_Process(t *testing.T) {
	...
	t.Run("unknown action returns error", func(t *testing.T) {
		...
		charge.AssertNotCalled(t, "RunChargeJob", mock.Anything, mock.Anything)
		refund.AssertNotCalled(t, "RunRefundJob", mock.Anything, mock.Anything)
	})
	...
	t.Run("charge action dispatches to ChargeProcessor", func(t *testing.T) {
		...
		charge.EXPECT().RunChargeJob(mock.Anything, job).Return(errors.New("charge failed"))
	...
	t.Run("refund action dispatches to RefundProcessor", func(t *testing.T) {
		...
		refund.EXPECT().RunRefundJob(mock.Anything, job).Return(nil)
