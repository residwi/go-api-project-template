...
`make check-boundaries` runs **six** checks and fails the build on any of
them. They are numbered 1, 2, 3, 4, 6 and 8; the gaps are where two checks were
...
5. *(retired)* — it refused a slice importing a sibling slice; there are no
   slices.
6. A module may not import `internal/server`, with no exemption — every
   middleware a handler needs lives in `internal/platform/web/middleware`.
7. *(retired)* — it kept each `contract/` package a leaf; published types live
   in `contract.go` in the module's root package now.
8. Nothing under `internal/platform` may import a local package outside
   `internal/platform`, so it stays copy-pasteable; `internal/testutil` is the
   one exemption.
