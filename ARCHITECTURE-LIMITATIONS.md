...
**Where you hit it:** you open `internal/bootstrap/app.go` and find 14 aliased adapter imports (`cartpg`, `orderpg`, `userredis`, …) above a 75-line `New`.
...
12 packages named `postgres` and 14 named `http` at each feature's own root, one named `redis` — 16 and 18 once you count slices' own (`shipping`'s four slices are the whole difference today; phase 2 raises both again with every module it slices next) — so every site wiring one needs an alias. That is also where this section's opening 14 comes from, down from 15: `shipping`'s four slices wire their own `postgres/` inside `shipping/module.go` now, so `app.go` lost the one `shippingpg` alias it used to carry and gained none in its place. `cmd/worker/main.go` needs 3 of its own, for the two job-queue repositories and the payment worker's processor. `ARCHITECTURE.md` §0 and §3 own this: in a product codebase the subpackage split would be hard to justify, here it is the point — physical boundary makes `payment` importing `payment/postgres` a compile error, not a convention.
...
**What it costs.** Nothing today. The cost is that the composition root is no longer the _only_ place `cmd/worker` reaches for a feature's adapter — `internal/bootstrap` remains the sole importer of `paymentpg`/`notificationpg` inside `internal/`, satisfying rule 3, but `cmd/worker/main.go` (outside `internal/`, so outside `check-boundaries.sh`'s reach entirely) also imports them directly, for the queue-side handle `App` cannot provide.
