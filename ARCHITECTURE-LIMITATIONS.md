...
**Where you hit it:** you swap `InventoryDeduct: inv.Deduct` for the wrong `inv` method (or drop the line) in `internal/bootstrap/app.go:100`, and `make check-boundaries`, `make lint`, `make test` and every `test/e2e` saga still pass.
...
**What breaks it.** Swapping `inv.Deduct` for `inv.Reserve` or `inv.Restore` on that one line still compiles, since `InventoryDeductor`, `InventoryReserver` and `InventoryRestorer` are structurally different single/two-method interfaces — but a future producer method that happens to satisfy `InventoryDeductor` by accident would not raise even a type error, only a wrong behaviour the first zero-total order hits.
