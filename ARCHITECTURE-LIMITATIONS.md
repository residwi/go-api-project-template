...
- **Redis: a hand-assigned integer, and now with no registry at all.** `MustStartRedis(dbIndex)` take an index the caller picks. It used to be picked against a comment block in `internal/testhelper/testhelper.go`; **that block is gone**, stripped along with every other comment in this phase's first commit and not replaced, so the record of who holds what is now only the five call sites: `grep -rn 'MustStartRedis(' --include='*_test.go' .` shows 0, 1, 3, 5, and 6 claimed (`platform/cache`, `transport/http/middleware`, `transport/http`, `test/e2e`, `modules/user/usecase/query/redis`); 2 and 4 sit free. `ResetRedis` call `FlushDB`, so reusing index flush other package's fixtures.
