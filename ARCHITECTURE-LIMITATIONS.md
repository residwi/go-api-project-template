...
`products` owned by `product`, `inventory_levels` owned by `inventory` (decision 6, decision 7), so listing query cannot join them. Port is `query.InventoryReader.GetAvailability(ctx, ids)` (`internal/modules/product/query/ports.go`) — batch-shaped, asked _after_ page already selected. `product/query.Reader.ListPublished` calls `repo.ListPublished` then `enrich`, that order, cannot be other order: enrich need ids page chose.
...
**Where you hit it:** you read `product/query.Reader.ListPublished` and count round trips.
...
updated_at`. **No price column**. `cart/query.Reader.GetCart` read lines then call `products.GetInfoByIDs` for current name, price, status, available stock. So cart not display stale price — never pinned one at all. Every read current.
...
Behaviour change, and chosen. Previous implementation drop line with `JOIN … AND p.deleted_at IS NULL`, so customer's total fell with nothing on screen to explain. If product record gone entirely, `cart/query.Reader.GetCart` substitute synthetic `&Product{Status: "unavailable"}` placeholder instead of dropping item.
...
**What it costs:** seam between `order` and `promotion` untyped in middle. `place.CouponReserver.Reserve` (`internal/modules/order/place/ports.go`) pass `orderSubtotal int64` and return `discountAmount int64`, and `order/place/command.go` re-pair it with `money.New(discount, subtotal.Currency)` on own side. That also where clamp live — `max(subtotal-discount, 0)` as plain arithmetic, because `Money.Sub` deliberately not decide whether money may go negative. If you add multi-currency promotion system, this seam where type safety missing and where wrong-currency discount would pass unnoticed.
...
- **`dashboard` is exempt wholesale.** Nothing verifies it stays read-only — no grant, no separate role, no check. An `UPDATE` in `internal/modules/dashboard/*/postgres` passes CI today. Nothing bounds it to today's two tables either, so "may read anything" can become a second, undeclared copy of the schema without a review comment.
...
Checks 1 and 3 are path-based: check 1 exempts `internal/modules/<feature>/<slice>/http/`, and check 3 (before this phase) scanned only a `postgres/` directory. From the moment a module was sliced until this phase rewrote both, every path either check keyed off had already moved one level deeper than the check expected, so both matched nothing in that module and reported nothing — verified during this phase by planting a `json` tag on a domain type and a foreign `FROM orders` in a command, in an already-sliced module, both of which passed a green `check-boundaries` before the fix.

**What you would do:** on the next structural migration, rewrite the checks against a hand-built pilot module first and run both shapes — old check, new tree — in parallel, rather than choosing to rewrite the checks at the end. The cost of the choice made here is that the first green run of the real checks came after every module had already moved, which is exactly backwards from when a boundary check is most useful.
...
Check 3 now scans every non-test `.go` file under a module, so SQL in a slice's `command.go` is caught exactly like SQL in its `postgres/` adapter — the hole the previous entry describes is closed. It still only matches string literals. A table name assembled with `+` or `fmt.Sprintf`, or reaching `pgx.CopyFrom` as a `pgx.Identifier` value, is invisible either way; widening which directories get scanned changes nothing about what the pattern itself can see.
...
`action='charge'`** — all three call sites that create a job (two in `payment/charge/command.go`, one in `payment/refund/command.go`) go through `jobs.Command.EnqueueRefund`, which hardcodes `domain.ActionRefund`. So `charge.Command.ProcessCharge` is unreachable outside tests, even though the dispatcher routes to it correctly.

**Why it looks otherwise.** Charging happens inline instead, on two paths: `charge.Command.InitiatePayment` finalises synchronously when the gateway captures funds immediately, and `webhook.Command`'s callback finalises when it does not. Both call `charge.Command.FinalizePaymentSuccess` with a **synthetic** `Job` carrying only `PaymentID`, `OrderID` and `Action` — no `ID`, because no row exists. Three consequences, individually invisible:
...
- **Redis: a hand-assigned integer.** `MustStartRedis(dbIndex)` take index from comment block in `internal/testhelper/testhelper.go`. Indices 0, 1, 3, 5, and 6 claimed today (`platform/cache`, `transport/http/middleware`, `transport/http`, `test/e2e`, `modules/user/query/redis`); 2 and 4 sit free. `ResetRedis` call `FlushDB`, so reusing index flush other package's fixtures.
...
**Where you hit it, the first way:** you move a route to the wrong `middleware.RouteGroup` in a feature's `routes.go` — admin instead of authed, or the reverse — and every test still passes.

A slice's own `handler_test.go` builds its own `middleware.NewRouteGroup` and writes the prefix itself, rather than importing anything from the real router. It is not even the same prefix: `internal/modules/category/create/http/handler_test.go:175` builds `middleware.NewRouteGroup(mux, "/api/v1/admin")`, while `internal/transport/http/router.go` mounts the real admin group at `/api/admin` — no `/v1` anywhere in production. The test passes either way, because it never touches the real router, only its own hand-built stand-in. `test/e2e` is the only thing that would catch a route landing on the wrong group, and only where a saga happens to exercise that exact route.
...
`testhelper.FakeTxRunner.Run` (`internal/testhelper/txrunner.go`) is `return fn(ctx)`: it calls the callback inline, with no transaction underneath it, so a mock-based `command_test.go` (order/cancel, order/expire, order/place, payment/charge, payment/refund, promotion/reserve, shipping/create, shipping/deliver, cart/add, at least) cannot observe whether a repository call happened inside `tx.Run`'s closure or leaked outside it — both look identical to the fake.

**What you would do:** for the first, either assert the mounted prefix somewhere real-router-shaped — a lightweight route-table snapshot test driven through `apihttp.NewRouter` itself — or accept that `test/e2e` is the only backstop and say so out loud, which is what this entry does. For the second, a real `TxRunner` backed by a test transaction (not the fake) would let a test assert call order across the boundary, at the cost of every affected `command_test.go` needing a real Postgres connection instead of a mock — trading a fast unit test for a slower, more honest one. Neither fix is free; neither is done here.
...
Phase 0 made `bootstrap.New` the single composition root, so the tedium moved there first — and then phase 2's slicing mostly moved it back out again, one level down. `app.go` no longer imports a module's `postgres`/`http`/`redis` adapter at all: it imports each module by its unaliased root package (`auth`, `cart`, `category`, …) and calls that module's own `New`, which wires its own slices' adapters inside its own `module.go`. The six aliases that remain — `ordercancel`, `ordercancelpg`, `ordertransition`, `ordertransitionpg`, `orderquery`, `orderquerypg` — exist only because breaking the order/payment cycle needs two pieces of `order` (`transition`, `query`) built before `order.New` itself can run, so `app.go` reaches one level past `order`'s module boundary for those two alone. `func New` is 84 lines (`internal/bootstrap/app.go:75`-`158`).

`internal/transport/http/router.go` kept its own pile: 14 aliased `http` imports, one per feature (`authhttp`, `carthttp`, …), plus the dev-only mock gateway's route registrar — 15 aliases above a 77-line `NewRouter`. Unlike `app.go`, this one has nowhere left to move: `router.go` mounts each feature's root `http/routes.go`, and that file is exactly what a router needs to import, sliced or not. `cmd/worker/main.go` needs one alias of its own now (`paymentworker`, for the processor that wraps payment's queue plus order's housekeeping sweep) — down from three, now that it reads `app.Payments.Jobs`, `app.Payments.JobProcessor` and `app.Notifications.Jobs` straight off `bootstrap.App` instead of building its own second `paymentpg`/`notificationpg` repository handles (see the removed "cmd/worker builds its own second handle" entry this phase deleted — `payment.Module` and `notification.Module` now expose exactly the split `Queue`/`Processor` pair the worker needs).

64 packages named `postgres` and 67 named `http` exist under `internal/modules` today, one named `redis` — re-run `find internal/modules -type d -name postgres | wc -l` (and `http`, `redis`) rather than trust these; they move with every slice added. Every one of them still needs an alias somewhere, just mostly inside the `module.go` that owns it now rather than at the top of the binary. `ARCHITECTURE.md` §0 and §3 own this: in a product codebase the subpackage split would be hard to justify, here it is the point — physical boundary makes a slice importing its own `postgres/` a compile error, not a convention.

**What it costs beyond ugliness:** adding a feature still means touching `app.go` once (one line to build the module, one field on `App`) and `router.go` once (one aliased import, one `RegisterRoutes` call) — smaller touches than before, but still two files every feature collides on. Neither `New` nor `NewRouter` carries a `//nolint:funlen`; both fit comfortably under the linter's 120-line limit, so the tedium here is pure import-alias noise, not the cognitive-complexity kind several slices' `command.go` files still carry `//nolint` for elsewhere in the tree (see AGENTS.md's Guardrails section for the current list). Cost concentrated in one file per binary-wide concern, deliberately.
...
- `cart_items` carries `UNIQUE (cart_id, product_id)`, and `cart/add.Command.Execute` upserts via `ON CONFLICT (cart_id, product_id) DO UPDATE` — a cart cannot hold two rows for one product.
- `order/place.Command.Execute` builds its reservation map one entry per cart-snapshot item, inheriting that guarantee directly — the map and the cart are keyed by the same product ids, one-for-one.
- `order_items` — read back by `place.Command.finalizeFreeOrder`, `cancel.Command.cancelWithReversal`, `expire.Command.releaseOrderHolds`, and payment's refund and charge-success paths via `order/query.Reader.ListItemsByOrderID` — has **no unique constraint on `(order_id, product_id)`**, only `PRIMARY KEY (id)` and a plain index on `order_id` (`db/migrations/20260424120005_create_orders.sql`). It is unique-per-product today only because the one write path that populates it (`place.Command.Execute` → `repo.CreateItems`, one row per cart-snapshot item) already can't produce duplicates. The invariant holds one level removed from any enforcement of its own.
...
**A second, sharper limit: nothing checks the single-naming invariant.** An attribute named at two points on one code path is emitted twice, and slog does not deduplicate keys. `payment/jobs.Dispatcher.Process` names `job_id` for the whole worker path; `charge.Command.FinalizePaymentSuccess` and `charge.Command.RunCompensatingRefund` are also reached from `charge.Command.InitiatePayment` and `webhook.Command.Execute`, which pass a `Job` literal with no `ID` and so deliberately name nothing. Add a fourth caller that names `job_id` itself and the worker path start emitting the key twice, with no test or linter to catch it. The check is a grep of the callers, run by hand.

This is not hypothetical. Naming `user_id` at the auth edge immediately collided with an `invalidateStatusCache` helper that logged its own `user_id` — the user being acted upon, not the caller. On an admin role change the record carried both, and a last-wins parser kept the admin's id while silently dropping the target's. The fix was to rename the inner one to `target_user_id`, because the two values answer different questions. That helper is now duplicated three times — `user/remove/command.go`, `user/adminupdate/command.go`, `user/updaterole/command.go` — one private copy per slice that needs it, each logging `target_user_id` the same way; the collision this paragraph describes had to be fixed in all three once slicing split them apart.
