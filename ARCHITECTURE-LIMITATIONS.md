...
**Where you hit it:** you drop the `InventoryDeduct: inv.Deduct` line in `internal/bootstrap/app.go:100` — a struct literal compiles fine with a field left unset, so `order.Deps.InventoryDeduct` is silently `nil` — and `make check-boundaries`, `make lint`, `make test` and every `test/e2e` saga still pass.

**Why it is safe today.** Splitting the old single `InventoryPort` into `InventoryReserver`/`InventoryDeductor`/`InventoryRestorer` gave `order` a `Deps.InventoryDeduct` field, wired at `app.go:100` to the same `inv.Deduct` value `payment.Deps.InventoryDeduct` already gets at `app.go:89`. Inside `order`, that field feeds only `place.Deps.Deductor` (`internal/modules/order/module.go:113`), and `place.UseCase` calls it from exactly one place — `finalizeFreeOrder` (`internal/modules/order/usecase/place/usecase.go:219`), reached only when `order.Total.Amount == 0`, a 100%-discount coupon or an entirely free line. `place/usecase_test.go`'s "zero total finalizes order without payment" subtest (line 565) exercises that branch, but against `NewMockInventoryDeductor`, which proves `finalizeFreeOrder` calls whatever it is handed, not that `app.go` handed it the right thing. `test/e2e/checkout_test.go`'s only coupon saga, `TestE2ECouponOrderFlow`, applies a 10% discount and asserts a 999 total (`checkout_test.go:355-360`); no `test/e2e` file builds an order whose total lands at zero. Before this phase, one `Inventory InventoryPort` field carried reserve, deduct and restore together, so any checkout that reached inventory at all — free or paid — reached it through that one wire. The split gave `order` a second, narrower wire to the same producer that only the free-order corner exercises.

**What it costs.** The other five cart/inventory fields on `order.Deps` (`CartLock`, `CartRead`, `CartClear`, `InventoryReserve`, `InventoryRestore`) are reached by every paid checkout `test/e2e/checkout_test.go` runs, so a mis-wire on any of those fails the suite immediately. `InventoryDeduct` is the one exception among order's ten fields: a copy-paste slip in the six-line block at `app.go:96-101` — exactly the kind of mistake a run of near-identical `inv.X` assignments invites — passes every check this repo runs and only misbehaves the first time a customer places a 100%-discount order in production.
