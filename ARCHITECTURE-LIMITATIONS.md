...
**Where you hit it:** you open `internal/bootstrap/app.go` expecting the pile of adapter aliases a template this size usually carries, and find one: `wishlistpg`. The six order/payment aliases this entry used to describe are still gone, but a flattened module's own adapter has nowhere else to hide.

Phase 0 made `bootstrap.New` the single composition root, so the tedium moved there first — phase 2's slicing moved it back out again, one level down, into each module's own `module.go` — and the in-progress flatten (`REFACTOR-PLAN.md`) starts moving a sliver of it back in: a flattened module has no `module.go` left to wire its own adapter inside, so `bootstrap.New` imports that module's `adapter/postgres` package directly and aliases it (`wishlistpg`, since the package itself is still named `postgres`, same as every sibling adapter) to build the `Repository` its `New` takes. The alias count is per adapter package, not per module, so it does not grow by exactly one per flatten task: `checkout`, already flattened, owns no store and reopens none; `user`, when it flattens, will reopen two (`userpg` and a cache alias), since `user/usecase/query` is the one slice backed by both Postgres and Redis today. `wishlist` (Task 6) is the only module flattened so far that owns a store, hence the one alias — read this count the same way as the others, stale until the refactor completes. Six aliased imports used to survive here regardless — `ordercancel`, `ordercancelpg`, `ordertransition`, `ordertransitionpg`, `orderquery`, `orderquerypg` — kept only because breaking the order/payment cycle needed three pieces of `order` (`usecase/transition`, `usecase/cancel`, `usecase/query`) built before `order.New` itself could run. With `order` needing nothing from `payment` any more, `order.New` now runs first and hands `payment.New` its own `Module.Transition`, `Module.Cancel` and `Module.Query` by name-match — one canonical value of each, not a second standalone copy — so `app.go` no longer reaches past a module boundary for order/payment's sake. `func New` is 74 lines (`internal/bootstrap/app.go:57`-`130`).
...
**Where you hit it:** you drop the `InventoryDeduct: inv.Deduct` line in `internal/bootstrap/app.go:77` — a struct literal compiles fine with a field left unset, so `order.Deps.InventoryDeduct` is silently `nil` — and `make check-boundaries`, `make lint`, `make test` and every `test/e2e` saga still pass.

**Why it is safe today.** Splitting the old single `InventoryPort` into `InventoryReserver`/`InventoryDeductor`/`InventoryRestorer` gave `order` a `Deps.InventoryDeduct` field, wired at `app.go:77` to the same `inv.Deduct` value `payment.Deps.InventoryDeduct` already gets at `app.go:91`. Inside `order`, that field feeds only `place.Deps.Deductor` (`internal/modules/order/module.go:98`), and `place.UseCase` calls it from exactly one place — `finalizeFreeOrder` (`internal/modules/order/usecase/place/usecase.go:187`), reached only when `order.Total.Amount == 0`, a 100%-discount coupon or an entirely free line. `place/usecase_test.go`'s "zero total finalizes order without payment" subtest (line 537) exercises that branch, but against `NewMockInventoryDeductor`, which proves `finalizeFreeOrder` calls whatever it is handed, not that `app.go` handed it the right thing. `test/e2e/checkout_test.go`'s only coupon saga, `TestE2ECouponOrderFlow`, applies a 10% discount and asserts a 999 total (`checkout_test.go:355-360`); no `test/e2e` file builds an order whose total lands at zero. Before this phase, one `Inventory InventoryPort` field carried reserve, deduct and restore together, so any checkout that reached inventory at all — free or paid — reached it through that one wire. The split gave `order` a second, narrower wire to the same producer that only the free-order corner exercises.

**What it costs.** The other five cart/inventory fields on `order.Deps` (`CartLock`, `CartRead`, `CartClear`, `InventoryReserve`, `InventoryRestore`) are reached by every paid checkout `test/e2e/checkout_test.go` runs, so a mis-wire on any of those fails the suite immediately. `InventoryDeduct` is the one exception among order's eight fields: a copy-paste slip in the six-line block at `app.go:73-78` — exactly the kind of mistake a run of near-identical `inv.X` assignments invites — passes every check this repo runs and only misbehaves the first time a customer places a 100%-discount order in production.

**What breaks it.** Swapping `inv.Deduct` for `inv.Reserve` or `inv.Restore` on that line does not compile: `InventoryReserver`, `InventoryDeductor` and `InventoryRestorer` are each single-method interfaces, but the three methods have three different names (`ReserveBatch`, `DeductBatch`, `Restore` — `reserve/usecase.go:19`, `deduct/usecase.go:19`, `restore/usecase.go:20`), so `*reserve.UseCase` and `*restore.UseCase` simply don't satisfy `InventoryDeductor` and the compiler catches that swap. The real risk is narrower: any future value — inside `inventory` or outside it — that happens to expose a `DeductBatch(ctx context.Context, items map[uuid.UUID]int) error` method satisfies `InventoryDeductor` with no type error at all, and only the untested zero-total path would notice a wrong one wired in.
