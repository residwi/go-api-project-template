...
`testhelper.FakeTxRunner.Run` (`internal/testhelper/txrunner.go`) is `return fn(ctx)`: it calls the callback inline, with no transaction underneath it, so a mock-based `usecase_test.go` or `service_test.go` cannot observe whether a repository call happened inside `tx.Run`'s closure or leaked outside it — both look identical to the fake. Seven files are in that position today (`grep -rl FakeTxRunner --include='*_test.go' internal/modules`): `order/usecase/{cancel,expire,place}`, still sliced, and `cart`'s, `shipping`'s, `promotion`'s and `payment`'s own merged `service_test.go` files — one file apiece standing in for what used to be `cart/usecase/add` alone, `shipping`'s `create` and `deliver`'s two, `promotion`'s `reserve` alone, and `payment`'s `charge` and `refund`, now that all four have flattened.
