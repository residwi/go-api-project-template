...
`NewRouter` itself, 65 routes in fifteen labelled blocks, mounted on
...
the real `NewRouter` and probes every entry in `allRoutes`, a hand-written
table of 65 `method<TAB>path` lines,
