...
- `internal/bootstrap/` — the composition root: builds every `Service` and wires every cross-module port
...
- `internal/platform/` — generic infrastructure, no feature deps:
  - `cache/` — Redis client
  - `config/` — infra config (`godotenv` + `envconfig`)
  - `database/` — pools, `TxRunner`, `PrimaryDB`/`ReplicaDB`, keyset and LIKE helpers
...
- `internal/modules/<feature>/` — the feature modules, plus two directories that are not features:
...
- **No comments in Go source, except directives and a comment that names a specific regression or a non-obvious constant.** Directives always stay — every `//nolint:` (with its justification on the same line) and every `//go:`. What survives beyond them today is `internal/modules/checkout/service.go`'s note above `if created && order.Total.Amount > 0` (without it the `created &&` guard reads as redundant, and deleting it reintroduces a double-charge bug) and `internal/modules/auth/service.go`'s notes on `dummyPassword` and `maxPasswordBytes` (bcrypt limits that the code cannot state itself). A comment earns its place by naming what breaks without it, not by explaining what the line already says.
...
1. **`check_wire_tags`** — a `json` tag lives only in a module's own `adapter/http`. `internal/modules/<feature>/http/` is not exempt, and neither is `internal/server`. `json:"-"` may not appear anywhere under `internal/`, and no file may be named `dto.go`. `internal/platform/` is exempt by location (the response envelope, the `envconfig` tags); `internal/modules/payment/adapter/gateway/gateway.go` is allowlisted by name, being the external gateway's wire contract rather than ours.
...
4. **`check_cross_module_imports`** — a module may import another module's root package, which is its published surface, and nothing deeper. `domain/` and every adapter stay private. Only the wiring layer (`bootstrap`, `server`, `worker`) may reach inside a module. One per-importer exemption exists: `checkout` alone may import a module's `domain/`, because `order.Service.Place`'s signature is written in `orderdomain` types.
...
Read "What it does not catch" in `db/OWNERSHIP.md` before trusting a green run. In short: table names must be string literals, `_test.go` files are skipped, `dashboard` is exempt wholesale, ownership is per table so column coupling is invisible, every check walks `internal/` only — `cmd/` and `test/` are outside all of them — and none of the checks is a compiler; they are all greps.

### Conventions the checks cannot see

- **A module imports another module's root package and nothing deeper**, and that means `payment` _can_ see `order.Place`. Nothing stops a module calling a sibling method no port of its own declares, and no check can tell the difference. Declare the interface the consumer needs in the consumer's own `ports.go` and wire it in `internal/bootstrap`; do not reach for a sibling's method because the import already compiles.
...
- **Middleware lives in two places, and the line is whether it names a feature module.** Check 8 answers that by reading imports rather than by judgment: nothing under `internal/platform` may import `internal/modules`. `internal/platform/web/middleware` holds panic recovery, request-ID injection, request logging, CORS, the user context (`UserContext`, `SetUserContext`, `GetUserContext`, `RequireUser`), `RequireRole` and rate limiting. `internal/server/auth.go` holds `authMiddleware` alone, because it names `auth.ClaimsView` and `user.AccountStatus`. A new compression or timeout middleware goes in platform; one that needs a module's type goes beside `authMiddleware`. A middleware needing an import platform may not have is the answer, not a reason to widen check 8.
...
- **Adding a module:** create `internal/modules/<feature>/` in the shape above, add a row per owned table to `db/OWNERSHIP.md`, wire it into `internal/bootstrap/app.go` (one line to build it, one field on `App`), mount its routes in `internal/server/router.go`, then run `make check-boundaries` — a new module with an `adapter/postgres` and no ownership row fails it by design.
