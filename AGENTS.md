...
- `internal/app/` — the composition root: builds every `Service` and wires every cross-module port, and maps `config` values onto the platform option structs (`PoolOptions`, `ReplicaPoolOptions`). `app.New` also takes the cache client and wraps the four cacheable repositories in their `adapter/redis` decorator when it is non-nil; `cmd/worker` passes `nil`, which turns off caching **and invalidation** for that process
...
- `internal/platform/` — generic infrastructure, no feature deps:
  - `cache/` — Redis client; `NewRedis` takes a `*redis.Options` and only warns on a failed ping, so the process always has a client that reconnects on its own. Plus `ReadThrough`, the read-through with singleflight barrier, TTL jitter and not-found placeholder, driven through the generic method `ReadThrough.Take[T]`; and `CircuitBreaker`, a `redis.Limiter` go-redis consults before it dials
  - `database/` — pools (`PostgresOptions`), `TxRunner`, `PrimaryDB`/`ReplicaDB`/`InTx`, keyset and LIKE helpers
  ...
  - `tracing/` — `Setup`, provider wiring driven by `OTEL_*` env vars; `Record`, the error-to-span-status classification every prologue calls; and the root-CLIENT-span filter that drops River's poll chatter
  - `slug/`, `storage/`
  - `identity/` — the `Identity` an authenticated caller carries (`UserID`, `Role`) and how it travels: `NewContext`, `FromContext`
  - `web/` — `Middleware`, `Chain`, `Router`; `web/request/` (`Bind`, `RequireUser`, `ParseUUIDParam`, the validator); `web/response/` (envelope, `HandleErr`, `CursorPage`); `web/middleware/` (CORS, logging, recovery, request ID, `ClientIP`, `Auth`, `Require`/`RequireRole`, `RateLimit`) — each is a constructor returning `func(http.Handler) http.Handler`, except `RequestID`, which needs no configuration and is the handler wrapper itself
...
internal/features/<feature>/
  ...
  adapter/
    ...
    redis/           the caching Repository decorator (category, product,
                     promotion, notification)
```

**The subpackage tree is deliberately non-uniform.** A module gets an `adapter/postgres` only if it has SQL, an `adapter/http` only if it has a route, a `ports.go` only if it reaches outside itself, a `contract.go` only if a struct of its own crosses a port. `auth` keeps no store at all and asks `user` for everything through one port. Do not tidy the tree into uniformity, and do not add a pass-through adapter package to fill a slot. Read `ls internal/features/<feature>/` for the module in front of you:
...
- **Every exported `Service` method that takes `ctx` opens a span.** Three lines, and the first must be the first statement in the body: `ctx, span := s.tracer.Start(ctx, "cart.Add")`, then `defer span.End()`, then `defer func() { tracing.Record(span, err) }()`, with a named `err` return. Reassign `ctx`, or every SQL span below detaches and the trace goes flat. Keep `span.End()` literal, or `spancheck` stops seeing it. Put the prologue inside an `if` and `govet`'s shadow check fails. Pass `err` to `defer` directly instead of closing over it and it records `<nil>` forever. Keep `defer span.End()` above the recorder: defers run last-registered-first, so `Record` must be registered second to run first — `RecordError` on an ended span is dropped. A method returning no error records nothing unless it swallows one: `payment.CompensateRefund` calls `tracing.Record(span, txErr)` where it logs. `auth.BuildTokenPair` takes no `ctx`, so it gets no span.
- **The tracer is a `Service` field built in `New`** from the module's full import path, never a package-level `var` — `gochecknoglobals` rejects that, and the field is one line.
- **A business span carries no attributes**, except an id that is its flow's natural search key and is not already in the URL. Today that is `order.id`, on `order.Place`, `payment.Charge` and `payment.SettleRefund`, nowhere else.
- **Spans anywhere else are automatic** — `otelhttp` for routes, `otelpgx` for SQL, `redisotel` for Redis, `otelriver` for jobs. Never write a span in a repository, a handler, a `domain/` package or a job worker; it would nest a second span around one that already exists.
- **Caching is a `Repository` decorator in `adapter/redis`, never a `Service` concern.** Four modules have one — `category`, `product`, `promotion`, `notification` — and `make check-arch` still refuses a `Service` importing `platform/cache`. Policy (singleflight barrier, TTL jitter, not-found placeholder, corrupt-entry drop) lives in `platform/cache`; key naming and encoding live in the decorator. **Write every port method out; never embed the wrapped `Repository`** — embedding makes a newly added read silently uncached instead of a compile error. A write invalidates with `Del` on a `context.WithoutCancel` context after the inner call returns; it never writes the new value into the cache.
- **Never serve a cached read inside a transaction.** Guard it with `database.InTx(ctx)` and fall through to the inner repository, the way `promotion.GetByCode` does: a transaction that prices a write from the row it read cannot take a stale one. Nothing enforces this — the standing tripwire is `product.GetByIDsIncludingDeleted`, which `order.Place` reaches through `cart.Snapshot` inside its transaction.
- **Redis is bounded by the socket timeouts, not by a context per command.** Dial, read, write and pool all default to `1s` and `MaxRetries` is `-1`, because go-redis reads replies on `context.Background()` unless `ContextTimeoutEnabled` is set, so a caller's deadline never bounds the part that hangs. `cache.CircuitBreaker` is wired as the cache client's `redis.Limiter`; `internal/server` builds a **second** client without it for `middleware.RateLimit`, since that middleware fails open and one shared fuse would let a slow cache switch off login and checkout throttling.
...
| `<feature>/adapter/**` | its own core and `domain/`, plus `database`, `web`, `jobqueue` where it enqueues, and `cache` where it caches |
...
The common components — `money`, `apperror`, `errs`, `identity`, `paging`, `slug`, `tracing` — are importable from anywhere and import nothing of ours, except `tracing`, which imports `errs`: a span's status is a function of the error's kind, and the kinds live there. A platform package is nearly as strict: `jobqueue` may name `database` and `web` may name `logger`, while `database`, `cache`, `storage`, `logger`, `config` and `testutil` reach only the common components.
...
- **`MustStartRedis(dbIndex)` takes an index you pick by hand.** Claimed today: 0 (`platform/cache`), 1 (`platform/web/middleware`), 2 (`features/category/adapter/redis`), 3 (`internal/server`), 4 (`features/product/adapter/redis`), 5 (`test/e2e`), 6 (`features/promotion/adapter/redis`), 7 (`features/notification/adapter/redis`). Nothing enforces a claim — a collision compiles, passes review and fails as a flake in an unrelated package — so `grep -rn 'MustStartRedis(' --include='*_test.go' .` is the authority, and update this list in the commit that takes an index.
