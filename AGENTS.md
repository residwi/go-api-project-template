...
routeless methods — and `cart`'s `lock`, which has no route either:
`order`'s checkout transaction calls its `LockCart` directly, the way it
calls `inventory`'s routeless slices, to serialize two concurrent checkouts
of one cart.
...
every other slice and every other module — and `order`'s `transition`,
`expire` and `recoverstale`, none of which has a route: `transition` holds
the eight `Mark*` appliers plus `Apply` and `UpdateStatus`, and every other
...
builds — while `expire` and `recoverstale` are worker-tick sweeps `cmd/worker`
drives directly on a plain ticker, the way it drives payment's and
notification's job queues, just without a queue behind either.
...
| `order`      | `http/` (routes.go only) plus `place/ query/ retrypayment/ cancel/ changestatus/ expire/ recoverstale/ transition/`; `place/`, `query/`, `retrypayment/`, `cancel/` and `changestatus/` each have their own `postgres/` and `http/` — `changestatus/`'s whole surface is one admin route, `PUT /orders/{id}/status` — `expire/`, `recoverstale/` and `transition/` each have `postgres/` but no `http/` of their own: `expire` and `recoverstale` are worker-tick sweeps `cmd/worker` drives directly, and `transition` is reached only through each consuming slice's own narrow port, never a route or an import — the sliced shape, see the two-shapes note above |
...
A missing `handler.go` at a feature's own `http/` root means one of two different things, and which one depends on the feature's shape. In a **layered** feature it means what it says: every route registers on the admin group, so the only handler really is an `adminHandler` — `payment` and `inventory` are the case, and there `ls` tells you the truth outright. In a **sliced** feature it tells you nothing at all: the root `http/` holds only `routes.go` by construction, whether or not the feature has a public surface. `shipping` has one — `query` — living in that slice's own `http/`; `dashboard` has none, because all three of its slices register on admin; and nothing about the root directory distinguishes those two. So read `ls internal/modules/<feature>/http/` together with the two-shapes note above, and once the note says a feature is sliced, go look inside its slices. Naming the features here would just go stale every time phase 2 converts another one; the note is the list.
