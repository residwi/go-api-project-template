...
platform server testutil worker`. Being infrastructure exempts a directory from
...
`server` and `worker`, the script's `WIRING_DIRS` — may import a module's internals, so
...
internal/modules/<feature>/
  ...
  queue.go           the outbound job-enqueue port, named Queue -- payment
                     and notification only (2), paired with adapter/jobs
  ...
  adapter/
    ...
    channel/         notification only -- the outbound Channel port's log
                     implementation
    jobs/            job args, InsertOpts and a river.Worker -- payment,
                     notification and order only (3); the only place in the
                     module that names river
```
...
`jobs.Worker` respectively — first collapsed into one `internal/platform/jobs/postgres`
store the platform layer owned, and that store is gone too now: background
jobs run on River, and what each module keeps of its own is an `adapter/jobs`
package instead. See rule 16.
...
4. **`check_cross_module_imports`: a module may import another module's root
   ...
   the only door. Exempt as importers: `internal/bootstrap/`, `internal/server/`
   and `internal/worker/` (the script's `WIRING_DIRS`) — empty that list and the
   check reports **35** imports in the wiring layer alone (17 from `bootstrap`,
   15 from `server`, 3 from `worker`). Same-module imports
...
16. **Background jobs run on River — one client, one shared insert helper, no hand-rolled queue.** `internal/platform/jobs` (`Job`/`Record`/`Store`/`Registry` and the `Runner`) is deleted entirely, along with the `job_queue` table and `internal/testutil/jobqueue.go`'s `FakeQueue`. In its place, `internal/platform/queue` holds exactly two functions: `NewInsertClient(db database.DB) (*river.Client[pgx.Tx], error)`, an insert-only client, and `Insert(ctx, client, db, args, opts) error`. `Insert` type-asserts `database.PrimaryDB(ctx, db)` to `pgx.Tx` and calls `InsertTx` when the caller is inside one — that assertion is the whole reason the function exists: without it, an enqueue commits even when the business write around it rolls back. Each of `payment`, `notification` and `order` gained an `adapter/jobs` package — the only place in the module naming `river` — holding the job's args type, its `InsertOpts()` (queue, max attempts, uniqueness) and a `river.Worker`. Payment and notification also declare an outbound port in their own root `queue.go`, and `adapter/jobs` supplies the enqueuer that satisfies it by name-match — `paymentjobs.Queue` against `payment.Queue`, the same relationship `postgres.Repository` has with `payment.Repository`. Order has neither, because its stale sweep no longer enqueues anything.

    **The port is `Queue`, unqualified**, the way `repository.go` declares `Repository` and `channel.go` declares `Channel`: a module has one queue the way it has one store. It was `RefundQueue` and `SendQueue` for one commit, and the qualifier was wrong for a reason worth recording — a second job kind in the same module forces either a rename or a second interface satisfied by the same adapter value. `user.StatusCache` stays qualified for the reason that does not apply here: a cache is a cache *of something*, and one module may hold several.

    `internal/worker` is new — the jobs analogue of `internal/server`. It owns the one working `river.Client` for the process (three queues, `payment` `notification` `order`, one `river.Worker` registered per kind), checks that `WORKER_RESCUE_AFTER` stays below `order.StaleProcessingThreshold`, and declares the periodic job that replaced the old self-scheduling sweep: `ExpireStaleJob`, `ScheduleExpireStale` and the timestamp dedup key are gone, and `order.adapter/jobs.ExpireStaleWorker` runs instead as a `river.PeriodicJob`, re-derived on every client start rather than stored as a row — the accepted consequence is that a long outage now yields one sweep on restart rather than a backlog of missed ones. `cmd/worker/main.go` is now a `main` that calls `worker.Run()`. `internal/bootstrap` builds one insert-only client (`queue.NewInsertClient(db)`) and hands it to payment's and notification's enqueuers; `bootstrap.App` carries no river type of its own.
...
20. **Request-scoped attributes are named once, at the edge.**
    ...
    to carry `request_id`. Two edges do this today: `middleware.RequestID`
    (`request_id`) and `middleware.Auth` (`user_id`). A job's own logging no
    longer goes through this mechanism: `internal/worker` hands River's
    `Logger` field the same `*slog.Logger` the rest of the process uses, and
    River names its own attributes (queue, kind, attempt, job ID) around it
    directly rather than through `logger.WithAttrs` — the `jobs.Runner.Start`
    (`runner`) and `Runner.processOne` (`job_id`) edges this rule used to name
    are gone along with `internal/platform/jobs`.
...
- **No comments in Go source, except directives and one named exception.** This is narrower than "explain why, not how": a comment strip removed every prose comment in the tree, and nothing has been allowed back in since except what earns its place by name, not by category. Directives always stay — every `//nolint:` (with its justification on the same line; see Guardrails) and every `//go:` — because those are instructions to the toolchain, not prose for a reader. One specific comment survives because deleting it lets a future edit reopen a bug this refactor already paid to close: `internal/modules/checkout/service.go`'s comment above `if created && order.Total.Amount > 0` (without it, the `created &&` guard reads as redundant, and deleting it reintroduces the double-charge `6c8bc0f` fixed). A second used to stand beside it — `internal/platform/jobs/runner.go`'s doc comment above `leaseSafetyDivisor`, protecting `lease - lease/5` from being tidied into a window where two workers run the same job — but that file, and the hand-rolled lease it explained, is gone now that jobs run on River; there is nothing left in this tree for the comment to guard. Do not add a second by analogy — a comment earns a place here by naming a specific regression it prevents, not by resembling this one.
...
- **Keep tests fast.** Use `bcrypt.MinCost` for password hashes in tests (`DefaultCost` costs ~250ms per hash) and group tests exercising real `Register` path. Use `testing/synctest` for ticker- and timeout-driven code — nothing in the tree needs it today (`internal/platform/jobs/runner_test.go`, its one user, is gone along with the hand-rolled runner it tested), but reach for it first if a future job brings ticker-driven code back rather than hand-rolling around real time. Note `synctest` cannot wrap `pgxpool` acquire, so test holding real pool must shrink intervals and timeouts instead. Give intentionally-broken clients short timeouts (`MaxRetries: 0`, `DialTimeout: 200 * time.Millisecond`) so error paths fail in milliseconds not seconds.
...
- `README.md` — endpoint reference and quick start. Its "Project Structure" section agrees with this file; both rewritten against real tree. Its environment table is **curated subset** — 6 variables absent, the whole Redis pool group (`REDIS_POOL_SIZE`, `REDIS_MIN_IDLE_CONNS`, `REDIS_DIAL_TIMEOUT`, `REDIS_READ_TIMEOUT`, `REDIS_WRITE_TIMEOUT`, `REDIS_POOL_TIMEOUT`) and nothing else. `WORKER_PRUNE_AGE` and `WORKER_PRUNE_LIMIT` used to widen that count to 8; both are deleted outright now that River owns its own maintenance, so there is nothing left for the table to omit. `.env.example` is the exhaustive list; verified against `envconfig` tags across `internal/platform/config/config.go` (infra) plus each module's own `config.go` (`auth`, `cart`, `notification`, `order`, `payment` — the five with env vars of their own).
