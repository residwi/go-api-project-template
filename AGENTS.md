...
3. **Nothing outside the wiring layer imports a feature's `postgres`, `http` or
   `redis` package.** Features and shared infrastructure alike; only `internal/bootstrap/` and `internal/transport/` may wire adapters together. Reaches one slice directory deeper too — `internal/modules/<feature>/<slice>/postgres` (also `http`, `redis`) is exactly as off-limits to every other module as the top-level adapter. A feature's own `module.go` (or any other file at its root) may still import its own slices' adapters — that is wiring the feature does for itself, the same permission a feature already had for its own top-level adapter — because the check compares the *imported* adapter's feature against the *importing* file's own feature, not against a filename; only a mismatch is reported, so the same mechanism refuses the import the moment it comes from a different feature.
