...
- `internal/platform/` — generic infrastructure, no feature deps:
  ...
  - `web/` — `Middleware`, `Chain`, `Router`; `web/request/` (`Bind`, `ParseUUIDParam`, the validator); `web/response/` (envelope, `HandleErr`, `CursorPage`); `web/middleware/` (CORS, logging, recovery, request ID, `Auth`, `RequireUser`, `Require`/`RequireRole`, `RateLimit`)
